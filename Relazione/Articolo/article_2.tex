...
\begin{equation}
AF(\textbf{r})=\sum_{m=1}^{N} a_{m} e^{jm \phi}
\end{equation}
...
\subsection*{Corporate feed}
La rete di alimentazione è composta da una combinazione di divisori a 3dB che distribuiscono la potenza alle 4 antenne.
Il divisore di potenza divide equamente la potenza senza perdite ma l'impedenza vista dall'ingresso sulle due uscite non è adattata ma equivale a $2Z_{0}$.
Si rende necessario quindi aggiungere dei trasformatori in $\frac{\lambda}{4}$ da $70 \Omega$ per adattare le uscite.
L'intera rete è riportata in Figura \ref{fig:Array4E_Schema}
\begin{figure}[h!]
